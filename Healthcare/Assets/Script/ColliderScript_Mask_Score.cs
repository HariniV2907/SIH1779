...
public class ColliderScript_Mask_Score : MonoBehaviour
{
    public string targetTag = "Mask";
    ...
    public bool isTask4Complete = false; // Task completion flag

    public AudioSource snapAudio; // Added AudioSource for snapping sound
        ...
        if (!isCooldown && other.CompareTag(targetTag) && sourceTransform != null && targetTransform != null)
        {
            if (IsCorrectlyPlaced())
            {
                if (!isTask4Complete)
                {
                    ...
                    isTask4Complete = true; // Task completed
                    Debug.Log("Task 4 is complete!");
